...
public:
	...
	virtual BOOL InitInstance(){
		std::wstring MsgFromDll;		// для хранения сообщения
		CMy82DLL* classObj;				// объект класса, объявленного в dll
		pGetDrivesType pGetDrives;		// для хранения указателя на функцию из dll
		pCreateType pCreate;			// для хранения указателя на функцию из dll
		...
		if (pGetDrives == NULL){
			...
			FreeModule(hDll);	// освободить память, которая выделена под dll, сделать dll недействительным
			return FALSE;
		};
